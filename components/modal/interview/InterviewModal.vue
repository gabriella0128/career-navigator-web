...
import { useModalStore, useInterviewStore, storeToRefs } from "#imports";
import type { ModalButton } from "~/types/modal";
const interviewStore = useInterviewStore();
const modalStore = useModalStore();
const { clearModal } = modalStore;

const { modalMode } = storeToRefs(interviewStore);
  ...
  <div v-if="isOpen" class="modal-overlay">
    <div class="modal-container">
      ...
      <div class="modal-body">
        <InterviewModalRegister v-if="modalMode === 'waiting'" :id="props.id" />
        <InterviewModalDetail v-if="modalMode === 'answered'" :id="props.id" />
      </div>
    </div>
  </div>
</template>
